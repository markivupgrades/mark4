...
def generate_index_thumbnail(symlink_path, thumb_path, size=(100, 150)):
    try:
        real_path = os.path.realpath(symlink_path)

        with Image.open(real_path) as img:
            img = img.rotate(-90, expand=True)  # Manual rotation only
            img.thumbnail(size)
...
def update_symlink(image_path):
    ...
    # ✅ Generate thumbnail for index page
    generate_index_thumbnail(
        symlink_path=SYMLINK_PATH,
        thumb_path=os.path.join(BASE_DIR, 'static', 'current_thumbnail.jpg')
    )
...
def select_folder():
    ...
    # 🛡️ Protect config write + delay flag
    with config_lock:
        save_config(config)
        Path(os.path.join(BASE_DIR, 'delay_updated.flag')).touch()
    ...
    # ✅ Use custom_order.txt if available
