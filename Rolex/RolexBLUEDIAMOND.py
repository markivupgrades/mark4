...
class Clock:
    def __init__(self):
        ...
        self.background_image = tk.PhotoImage(file=os.path.join(base_path, "RolexBLUEDIAMOND.png"))
        self.weekday_images = {
    ...
    def draw_date(self, day, weekday):
        self.canvas.create_image(984, 136, image=self.weekday_images[weekday], anchor='center')
        self.canvas.create_text(1407, 642, text=day, font=('Copperplate', 64), fill='black')
        self.canvas.create_text(1405, 640, text=day, font=('Copperplate', 64), fill='#FCF2E3', anchor='center')
    ...
    def draw_hands(self, hour, minute):
        ...
        hour_marker_start_x = x0 + (hour_x - x0) * ((310 - 60) / 310)
        hour_marker_start_y = y0 + (hour_y - y0) * ((310 - 60) / 310)
        hour_marker_end_x = x0 + (hour_x - x0) * ((310 - 15) / 310)
        hour_marker_end_y = y0 + (hour_y - y0) * ((310 - 15) / 310)
        self.canvas.create_line(hour_marker_start_x, hour_marker_start_y, hour_marker_end_x, hour_marker_end_y, fill='#FCF2E3', width=14, capstyle=tk.ROUND, tags='main_hands')

        minute_marker_start_x = x0 + (minute_x - x0) * ((520 - 70) / 520)
        minute_marker_start_y = y0 + (minute_y - y0) * ((520 - 70) / 520)
        minute_marker_end_x = x0 + (minute_x - x0) * ((520 - 20) / 520)
        minute_marker_end_y = y0 + (minute_y - y0) * ((520 - 20) / 520)
        self.canvas.create_line(minute_marker_start_x, minute_marker_start_y, minute_marker_end_x, minute_marker_end_y, fill='#FCF2E3', width=12, capstyle=tk.ROUND, tags='main_hands')
    ...
    def run(self):
        ...
        self.root.after(3600000, self.root.destroy)
        self.root.mainloop()
